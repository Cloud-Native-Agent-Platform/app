...
func TestControllerCreateTaskWithPrompt(t *testing.T) {
	...
	// Task 생성 with prompt
	require.NoError(t, ctrl.CreateTask(ctx, "chatbot", "task-001", "Test Task", "Hello, how are you?", nil))
...
func TestControllerCreateTaskWithoutPrompt(t *testing.T) {
	...
	require.NoError(t, ctrl.CreateTask(ctx, "agent-1", "task-empty", "", "", nil))
...
func TestControllerAddMessage(t *testing.T) {
	...
	// Setup
	require.NoError(t, ctrl.CreateAgent(ctx, "agent-1", "Test agent", "gpt-4", "System prompt"))
	require.NoError(t, ctrl.CreateTask(ctx, "agent-1", "task-001", "", "Initial prompt", nil))
...
func TestControllerSendMessage(t *testing.T) {
	...
	// Setup
	require.NoError(t, ctrl.CreateAgent(ctx, "agent-1", "Test agent", "gpt-4", "System prompt"))
	require.NoError(t, ctrl.CreateTask(ctx, "agent-1", "task-001", "", "Hello", nil))
...
func TestControllerMultiTurnConversation(t *testing.T) {
	...
	// 2. Task 생성 (대화 세션)
	require.NoError(t, ctrl.CreateTask(ctx, "chatbot", "session-001", "", "안녕하세요", nil))
